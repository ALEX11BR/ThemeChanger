...
def installThemeArchive(kind, archivePath, destination, anchorFile, anchorLevels, lookupFunction):
    ...
    # Then, we create a tempdir somewhere on the user's cache to have it on the same partition as the theme install destination
    try:
        with tempfile.TemporaryDirectory(dir=GLib.get_user_cache_dir()) as tempDirPath:
            ...
            # An anchorFile can be 'gtk-3.0/gtk.css' for widget themes or 'index.theme' for icon themes.
            # In the first case, anchorLevels is 2, and in the second one is 1.
